...
## Custo de transação: 100% no desembolso ou amortizado (CPC 08 (R1), item 12)

Flag por contrato (Calculadora, logo abaixo de "Taxas financiadas"), revogando a parte "nunca por empresa" da decisão D8 — agora é por contrato, não mais só ferramenta inteira. Padrão continua **"100% no desembolso"** (`transaction_cost_recognition = 'imediato'`), zero mudança de comportamento nos contratos existentes.
- **Base normativa (conferida na fonte, CVM):** CPC 08 (R1), item 12 — "Os encargos financeiros incorridos na captação de recursos junto a terceiros devem ser apropriados ao resultado em função da fluência do prazo, pelo custo amortizado usando o método dos juros efetivos." O método exato da norma é a taxa efetiva (TIR), recalculando a taxa implícita do contrato inteiro — não é o que o sistema faz.
- **Simplificação adotada ("Amortização conforme CPC-08" na tela):** linha reta por dias corridos, do desembolso ao fim do prazo, no mesmo padrão de rateio da apropriação de juros por competência. A tela deixa isso explícito na nota abaixo da flag, para não prometer conformidade literal com a norma.
- **Partidas:** no desembolso, o custo vai para uma conta de **ativo diferido** (evento `custo_transacao_diferido`, novo) em vez de despesa direto. Todo mês, `custo_transacao_apropriacao` (revivido — existia e foi descontinuado antes da decisão D8) debita despesa financeira e credita a MESMA conta de ativo diferido, até zerar no fim do prazo.
- **Quitação antecipada ou renegociação** (`payoff_date` cai no mês): reconhece de uma vez o saldo do ativo diferido ainda não apropriado, na data real da baixa — não continua o rateio mês a mês.
- **Lógica Contábil:** os dois eventos exigidos por categoria só quando algum contrato daquela categoria usa "amortizado" (`deploymentReadiness.js`).
- **Limitação conhecida:** contrato em implantação de saldos com a política "amortizado" não tem o saldo do ativo diferido representado na posição de abertura (aviso na tela). Migration 073.
