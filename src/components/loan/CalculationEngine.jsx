 ...
 */
export const ENGINE_VERSION = "1.2.1"; // +ETAPA 3.1: Governance Hardening (PTAX_GAP, Mutation Guard)
 ...
 */
export const ENGINE_BUILD_ID = "build-20260222-fix-imports"; // +FIX: Removido require() - ES6 imports
...
import { AMERICANOStrategy } from "./strategies/AMERICANOStrategy";
import { BULLETStrategy } from "./strategies/BULLETStrategy";
import { PERCENTAGE_RESIDUAL_Strategy } from "./strategies/PERCENTAGE_RESIDUAL_Strategy";
import { IndexerFactory } from "./indexers/IndexerFactory";
import { roundMoney, roundPercent, roundExchangeRate } from "./roundMoney";
...
import { PrecisionAudit } from "./PrecisionAudit";
import * as ScheduleIntegrity from "./ScheduleIntegrity";
import { AuditLog } from "./AuditLog";
import { createPrecisionGovernance } from "./PrecisionGovernance";
...
function addMonths(date, months) {
  ...
  const nominalDay = d.getDate();
  
  // Define o dia como 1 antes de mudar o mês para evitar overflow imediato
  d.setDate(1);
  d.setMonth(d.getMonth() + months);
  
  // Busca o último dia do mês alvo
  const lastDayOfTargetMonth = new Date(d.getFullYear(), d.getMonth() + 1, 0).getDate();
  
  // Se o dia nominal (ex: 31) não existe no mês alvo, trava no último dia (ex: 28)
...
// Gera datas de vencimento usando dia de referência
// Se endDate é fornecido, calcula regressivo; senão, progressivo a partir de startDate
function generateDueDates(startDate, count, frequencyMonths, referenceDayOfMonth = null, endDate = null) {
  const dates = [];
  const dateReference = endDate ? new Date(endDate) : new Date(startDate);
  ...
  for (let i = 1; i <= count; i++) {
    let d;
    if (isRegressive) {
      // Regressivo: de endDate, subtrair (count - i) frequências
      const offsetMonths = (count - i) * frequencyMonths;
      d = addMonths(new Date(dateReference), -offsetMonths);
    } else {
      // Progressivo: de startDate, adicionar i frequências
      d = addMonths(new Date(startDate), i * frequencyMonths);
    }
    // Ajustar para o dia de referência
    d.setDate(refDay);
    // Se o mês não tem esse dia (ex: 31 em fev), usar último dia do mês
    if (d.getDate() !== refDay) {
      d.setDate(0); // Volta para último dia do mês anterior
    }
    dates.push(nextBusinessDay(d));
  }
  return dates;
}
...
import { validateLegalCompliance, validateCalculationContext } from "./LegalComplianceValidator";
...
export async function calculateAmortizationSchedule(params) {
  ...
  const {
    ...
    firstPaymentDate: rawFirstPaymentDate, // Data do primeiro pagamento
  ...
  const firstPaymentDate = normalizeDate(rawFirstPaymentDate);
  ...
  // CRÍTICO: Garantir que data seja parseada sem conversão de timezone
  const startDate = new Date(operationDate + 'T00:00:00');
  ...
  let referenceDayOfMonth = null;
  let totalMonths = 0;
  
  // Se finalMaturityDate existe, usar apenas para referência de dia e validação final
  const hasFinalDate = finalMaturityDate && String(finalMaturityDate).trim() !== "";

  if (hasFinalDate) {
    const finalDate = new Date(finalMaturityDate + 'T00:00:00');
    referenceDayOfMonth = finalDate.getDate();
    
    // USAR totalTermMonths se foi fornecido
    if (totalTermMonths && totalTermMonths > 0) {
      totalMonths = totalTermMonths;
    } else {
      // Caso contrário, calcular a diferença em meses
      const yearDiff = finalDate.getFullYear() - startDate.getFullYear();
      const monthDiff = finalDate.getMonth() - startDate.getMonth();
      totalMonths = yearDiff * 12 + monthDiff;
    }
  } else {
    // Usar dia original da operação como referência (ex: 31)
    referenceDayOfMonth = startDate.getDate();
  }
  // Nota: O campo "firstPaymentDate" é IGNORADO no cálculo - serve apenas como referência visual
  ...
  if (customDates && customDates.length > 0) {
    ...
    for (let i = 1; i < customDates.length; i++) {
      const prev = new Date(customDates[i - 1]);
      const curr = new Date(customDates[i]);
      if (curr <= prev) {
        throw new Error(
    ...
    for (let monthIdx = 0; monthIdx < customDates.length; monthIdx++) {
      ...
      mergedEvents.push({
        date: new Date(customDates[monthIdx]),
    ...
    // Lógica normal: calcular datas com ANCORAGEM
    // Sempre calcular a partir da data original somando os meses do índice
    ...
    for (let monthIdx = 0; monthIdx < totalMonths; monthIdx++) {
      ...
      let monthDate;
      const isLastPayment = monthIdx === totalMonths - 1;
      
      // SEMPRE calcular progressivo a partir da data de operação
      // A data final serve apenas para validação e ajuste do último pagamento
      if (isLastPayment && hasFinalDate) {
        // Última parcela: usar exatamente a data final configurada
        monthDate = new Date(finalMaturityDate + 'T00:00:00');
      } else {
        // Todas as outras: progressivo a partir da data de operação
        monthDate = addMonths(startDate, monthIdx + 1);
      }
      
      const dueDate = nextBusinessDay(monthDate, holidays);
      
      console.log(`📅 Gerando Mês ${monthIdx + 1}/${totalMonths}:`, {
        monthIdx,
        calculando: 'PROGRESSIVO',
        startDate: startDate.toISOString().split('T')[0],
        monthDate: monthDate.toISOString().split('T')[0],
        dueDate: dueDate.toISOString().split('T')[0],
        hasPrincipal,
        hasInterest,
        finalMaturityDate: hasFinalDate ? finalMaturityDate : 'N/A'
      });
  ...
  for (let i = 0; i < mergedEvents.length; i++) {
    ...
    if (isUSD) {
     ...
     const ptaxData = getExchangeRate(evt.date.toISOString().split('T')[0], exchangeLag, safeExchangeRates);
     ...
     if (!currentPtaxRate || currentPtaxRate <= 0 || isNaN(currentPtaxRate)) {
       throw new Error(
         `Parcela ${parcela}: Taxa PTAX inválida (${currentPtaxRate}) para data ${evt.date.toISOString().split('T')[0]}. ` +
    ...
    if (i < 3 || principalGraceMonths > 0) {
      console.log(`📊 Parcela ${parcela}:`, {
        ...
        currentDate: evt.date.toISOString().split('T')[0],
    ...
    if (indexer !== "NA") {
     ...
     const indexAccum = indexerFactory.getFactor(indexer, prevDate.toISOString().split('T')[0], evt.date.toISOString().split('T')[0], holidays);
    ...
    schedule.push({
      id: generateUUID(),
      parcela,
      dataVencimento: evt.date.toISOString().split("T")[0],
